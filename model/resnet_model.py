...
class ResNet3D(nn.Module):
    def __init__(self, depth: int, num_classes: int = 4,
                 weights_loc: str = "./ft_weights"):
        
        # download the FT weights in the directory 
        url = 'https://drive.google.com/uc?id=1uGkkk57kUADfU6xkGjItomEftQZOAj4v'
        os.makedirs(weights_loc, exist_ok=True)
        output = os.path.join(weights_loc, 'r3d18-kinetics.pth')
        gdown.download(url, output, quiet=False)
        super(ResNet3D, self).__init__()
        assert depth in [18]  #, 34, 50]

        # model = generate_model(model_depth=depth, sample_size=224, sample_duration=30)
        weights_file = os.path.join(weights_loc, 'r3d18-kinetics.pth')
        ...
        model = load_pretrained_model(model=model, pretrain_path=weights_file,
